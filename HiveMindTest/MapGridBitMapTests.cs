...
{
    public class MapGridBitMapTests
    {
        ...
        public void Setup()
        {
            ...
            // Test grids
            MapGrid.GetDataValueBit(_gameInfo.StartRaw.PathingGrid, 22, 131).Should().Be(1);
            MapGrid.GetDataValueBit(_gameInfo.StartRaw.PlacementGrid, 22, 131).Should().Be(1);
            MapGrid.GetDataValueBit(_gameInfo.StartRaw.PathingGrid, 40, 124).Should().Be(1);
            MapGrid.GetDataValueBit(_gameInfo.StartRaw.PlacementGrid, 40, 123).Should().Be(0);
        }

        [Test]
        public void MapGridSetup()
        {
            var mapGrid = new MapGrid(_gameInfo.StartRaw.PlacementGrid, _gameInfo.StartRaw.PathingGrid, _gameInfo.StartRaw.PlayableArea);

            mapGrid.PlayableGrid[20, 130].Should().Be(Ground.Airspace);
            mapGrid.PlayableGrid[21, 130].Should().Be(Ground.BuildingPlacable);
            mapGrid.PlayableGrid[39, 123].Should().Be(Ground.Pathable); // Top left of ramp
            mapGrid.PlayableGrid[37, 118].Should().Be(Ground.Pathable); // Bottom right of ramp (ramp points to left)
        }

        //[Test]
        //public void TestImageDataManipulation()
        //{
        //    var mapGrid = new MapGrid(_gameInfo.StartRaw.PlacementGrid, _gameInfo.StartRaw.PathingGrid, _gameInfo.StartRaw.PlayableArea);

        //    var baseArea = mapGrid.SetBasePlateau(_gameInfo.StartRaw.StartLocations[0]);

        //    baseArea.X.Should().Be(22);
        //    baseArea.Y.Should().Be(137);
        //}
